...
int bit_check(int data, int bits_on, int bits_off){
  int test = bits_on & bits_off;

  if(test != 0){
    return -1;
  }

  int check_on = data & bits_on;
  int check_off = data & bits_off;
...
for(int i = 0; i < sizeof(a_array) / sizeof(a_array[0]); i++) {
        if(a_array[i] == c) {
            return &a_array[i]; // Return the pointer to the matching element
        }
    }
}
...
void floats_to_ints(float f, int* output) {
    output[0] = (int)f; // Integer part
    output[1] = (int)((f - output[0]) * 100); // Fractional part as two digits
}
...
char* per_print(Person* p, char* personbuffs){
  ...
  int heightF[2];
  int weightF[2];
  floats_to_ints(p->Height, heightF);
  floats_to_ints(p->Weight, weightF);

  //formatted[0] = '\0';
  sprintf(buffer, "--- person report: ----\n");
  strcat(personbuffs, buffer);

  sprintf(buffer, "%-*s %-*s\n", 30, "First Name:", 20, p->FirstName);
  strcat(personbuffs, buffer);
  
  sprintf(buffer, "%-*s %-*s\n", 30, "Last Name:", 30, p->LastName);
  strcat(personbuffs, buffer);

  if (strlen(p->StreetAddr) > 60) {
        strncpy(buffer2, p->StreetAddr, 60);
        buffer2[60] = '\0'; // Ensure null termination
        strcat(buffer2, "...");
  } else {
        strcpy(buffer2, p->StreetAddr);
  }
  sprintf(buffer, "%-*s %-*s\n", 30, "Address:", 60, buffer2);
  strcat(personbuffs, buffer);

  sprintf(buffer, "%-*s %-*d\n", 30, "Zip:", 6, p->ZipCode);
  strcat(personbuffs, buffer);

  sprintf(buffer, "%-*s %-*d.%02d\n", 30, "Height:", 5, heightF[0], heightF[1]);
  strcat(personbuffs, buffer);

  sprintf(buffer, "%-*s %-*d.%02d\n", 30, "Weight:", 5, weightF[0], weightF[1]);
  strcat(personbuffs, buffer);

  sprintf(buffer, "%-*s %-*d\n", 30, "DOB 1/1/1900:", 30, p->DBirth);
  strcat(personbuffs, buffer);
